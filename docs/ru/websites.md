...
## Продажа контейнеров (Multi-Tenant)

dck подходит для сценария продажи изолированных контейнеров клиентам — например,
игровых серверов (Minecraft, CS2), хостинга сайтов или сред разработки.

### Изоляция

Каждый контейнер получает:

- **Jailed rootfs** — контейнер видит только свои файлы через overlay
- **Уникальный SSH/SFTP порт** (22000+, изолирован от других)
- **Уникальный FTP порт** (23000+, изолирован от других)
- **Уникальную SSH ключевую пару** — клиент получает свой ключ
- **Динамические порты** — можно добавлять порты для игр, донатных привилегий и т.д.

### Пример: продажа Minecraft серверов

```bash
# Арендатор №1 — Minecraft сервер с SFTP доступом
...
# Показать SSH ключ для арендатора
dck sshkey player1

# Арендатор подключается:
# ssh -p 22001 -i player1_key dck@your-server
# sftp -P 22001 dck@your-server  # пароль: ID контейнера[:16]

# Позже добавить донатный порт (например, для плагина)
dck port add player1 25566:25566

# Арендатор №2
dck run -d --sftp --name player2 \
  -p 25567:25567 \
  --memory 2G --cpus 2 \
  -e EULA=TRUE -e TYPE=PAPER \
  itzg/minecraft-server
...
| Сценарий | Пример |
|---|---|
| **Игровой хостинг** | Minecraft, CS2, Terraria — каждый сервер в своём контейнере с --sftp |
| **Shared хостинг** | PHP/Node.js сайты с FTP доступом для клиентов |
| **Dev environment** | Каждому разработчику изолированный контейнер с SSH |

### Динамические порты для донатов

```bash
# Администратор может добавить порт без перезапуска
dck port add player1 27015:27015/udp   # CS2 голосовой чат
dck port add player1 25566:25566       # Доп. порт Minecraft
```

Правила применяются мгновенно через iptables DNAT и сохраняются после перезапуска.

### Преимущества для продажи контейнеров

- **Нет Docker** — меньше ресурсов, выше плотность на сервере
- **Лимиты ресурсов** — `--memory`, `--cpus`, `--disk` для каждого арендатора
- **Полная изоляция** — каждый видит только свой overlay
- **SFTP/FTP из коробки** — клиенты сами загружают файлы
- **Безопасность** — chroot, нет общего демона
