...
- [SSH, SFTP и FTP доступ](#ssh-sftp-и-ftp-доступ)
...
| `--sftp` | Встроенный SSH+SFTP сервер (порт 22000+, chroot в rootfs контейнера) |
| `--ftp` | Встроенный FTP сервер (порт 23000+, chroot в rootfs контейнера) |
...
## SSH, SFTP и FTP доступ

### Встроенный SSH + SFTP сервер

`dck run --sftp <образ>` запускает встроенный SSH+SFTP сервер для контейнера.
Сервер изолирует пользователя в rootfs контейнера через chroot (overlay).
Поддерживает как SSH-терминал, так и SFTP для передачи файлов.

```bash
dck run -d --sftp --name web nginx:alpine
```
...
# Пароль: ID контейнера (первые 16 символов)
```

**Характеристики:**
- Авто-генерация SSH ключевой пары на каждый контейнер
- Аутентификация: SSH ключ (основной) или пароль (ID контейнера, fallback)
- Использует `nsenter` для предоставления shell внутри контейнера
- Диапазон портов: 22000+
...
Ключи хранятся в `~/.dck/keys/<id-контейнера>_rsa`.

### Встроенный FTP сервер

`dck run --ftp <образ>` запускает встроенный FTP сервер, изолированный в rootfs контейнера.

```bash
dck run -d --ftp --name web nginx:alpine
```
...
- Пользователь: `dck`
- Пароль: ID контейнера (первые 16 символов)
- Изоляция: chroot в rootfs контейнера
- Диапазон портов: 23000+
