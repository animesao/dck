...
def _create_data_launcher(container_name, image, vol_path, ports, env_vars, ram, cpu):
    lines = [
        "#!/usr/bin/env bash",
        f"# {container_name} - Docker game server launcher",
        "# Generated by dck",
        "",
    ...
    for c_port, h_port in (ports or {}).items():
        ...
        lines.append(f"  -p {h_port}:{c_num}/{proto} \\")
    ...
    lines += [
        ...
        '  "$IMAGE"',
        "",
        f'echo "==> Server started. Use \'dck console {container_name}\' to connect."',
        f'echo "    Or: docker attach {container_name}"',
    ]
...
def build_and_start(image, name, ports, env_vars, volumes, ram, cpu, start_now=True, paper_info=None):
    ...
    if container.status == "running" and not is_game:
        if Confirm.ask(f"\n  Enter console?", default=True):
            from dck.exec import console_container
            console_container(container_name, mode="attach")
